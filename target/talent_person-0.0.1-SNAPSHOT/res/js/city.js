...
function changew(wl) {
	var str = "0";
	for (i = 0; i < wl; i++) {
		str += ("_" + (document.getElementById(w[i]).selectedIndex - 1));
	};
	var ss = document.getElementById(w[wl]);
	with (ss) {
		length = 0;
		options[0] = new Option(opt0[wl], opt0[wl]);
		if (wl && document.getElementById(w[wl - 1]).selectedIndex > 0 || !wl) {
			if (dsy.Exists(str)) {
				ar = dsy.Items[str];
				for (i = 0; i < ar.length; i++) options[length] = new Option(ar[i], ar[i]);
				if (wl) options[1].selected = true;
			}
		}
		if (++wl < w.length) { changew(wl); }
	}
}
...
function setupw() {
	for (i = 0; i < w.length - 1; i++){
		document.getElementById(w[i]).onchange = new Function("changew(" + (i + 1) + ")");
	}
	changew(0);
}
